...
<div class="max-w-7xl mx-auto">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-900 flex items-center gap-3">
                ...
                Laporan Stok Unit Global
            </h2>
            <p class="text-sm text-gray-500 mt-1 ml-4">Pantau ketersediaan unit motor di seluruh lokasi (Real-time).</p>
        </div>

        <a href="{{ route('laporan.stok.global.print', ['search' => $search]) }}" target="_blank" class="bg-gray-800 text-white font-bold py-2.5 px-6 rounded-lg shadow-md hover:bg-gray-900 transition-all flex items-center gap-2">
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        <form action="{{ route('laporan.stok.global') }}" method="GET" class="w-full flex flex-col sm:flex-row items-center gap-3">

            <div class="relative w-full sm:w-80">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </div>
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari Kode / Tipe Motor..." class="w-full border border-gray-300 rounded-lg py-2 pl-9 pr-4 outline-none focus:border-honda-red text-sm">
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    @forelse($stokTypes as $index => $item)
                        <tr class="hover:bg-red-50/30 transition-colors">
                            <td class="py-3 px-4 text-center text-gray-500 border-r border-gray-100">{{ $stokTypes->firstItem() + $index }}</td>
                            <td class="py-3 px-4 font-bold text-gray-700 border-r border-gray-100">{{ $item->kode_tipe }}</td>
                            <td class="py-3 px-4 font-bold text-gray-800 border-r border-gray-100">{{ $item->nama_type }}</td>

                            <td class="py-3 px-4 text-center font-semibold text-blue-600 border-r border-gray-100">{{ $item->stok_gudang }}</td>
                            <td class="py-3 px-4 text-center font-semibold text-emerald-600 border-r border-gray-100">{{ $item->stok_showroom }}</td>
                            <td class="py-3 px-4 text-center font-semibold text-amber-600 border-r border-gray-100">{{ $item->stok_pop }}</td>
                            <td class="py-3 px-4 text-center font-semibold text-purple-600 border-r border-gray-100">{{ $item->stok_gp }}</td>

                            <td class="py-3 px-4 text-center font-black text-gray-900 bg-gray-50">{{ $item->stok_total }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="py-12 text-center text-gray-400 italic">Tidak ada data stok tersedia.</td>
                        </tr>
                    @endforelse
                </tbody>
                <!-- Footer Total -->
                <tfoot class="bg-gray-100 font-black text-gray-900 text-sm border-t-2 border-gray-300">
                    <tr>
                        <td colspan="3" class="py-4 px-4 text-right uppercase border-r border-gray-200">Total Keseluruhan :</td>
