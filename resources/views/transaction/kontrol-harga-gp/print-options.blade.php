...
<div class="max-w-5xl mx-auto">
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-extrabold text-gray-900 flex items-center gap-3">
            ...
            Opsi Cetak Kwitansi GP
        </h2>
        <a href="{{ route('kontrol-harga-gp.index') }}" class="text-gray-500 hover:text-honda-red text-sm font-bold flex items-center gap-1">
            &larr; Kembali
        </a>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-6 text-center">
        <div class="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Konsumen (GP)</div>
        <h3 class="text-xl font-black text-gray-900 uppercase">{{ $spk->nama_stnk }}</h3>
        <div class="text-sm text-gray-600 mt-1">No. GPK: <span class="font-bold">{{ $spk->no_spk }}</span></div>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">

        <a href="{{ route('kontrol-harga-gp.print.otr', $spk->id) }}" target="_blank" class="block bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-500 transition-all group text-center">
        ...
        @if(strtolower($spk->jenis_pembayaran) === 'kredit' || !empty($spk->leasing_id))
            ...
            <a href="{{ route('kontrol-harga-gp.print.dp-po', $spk->id) }}" target="_blank" class="block bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-green-500 transition-all group text-center">
            ...
            <a href="{{ route('kontrol-harga-gp.print.otr-dp-po', $spk->id) }}" target="_blank" class="block bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-purple-500 transition-all group text-center">
            ...
            <a href="{{ route('kontrol-harga-gp.print.setoran-spk', $spk->id) }}" target="_blank" class="block bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-yellow-500 transition-all group text-center">
            ...
            <a href="{{ route('kontrol-harga-gp.print.surat-pernyataan-bpkb', $spk->id) }}" target="_blank" class="block bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-orange-500 transition-all group text-center">
            ...
            @if(isset($kontrol) && $kontrol->dll_1 > 0)
                <a href="{{ route('kontrol-harga-gp.print.kw1', $spk->id) }}" target="_blank" class="block bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-pink-500 transition-all group text-center">
            ...
            @if(isset($kontrol) && $kontrol->dll_2 > 0)
                <a href="{{ route('kontrol-harga-gp.print.kw2', $spk->id) }}" target="_blank" class="block bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-teal-500 transition-all group text-center">
