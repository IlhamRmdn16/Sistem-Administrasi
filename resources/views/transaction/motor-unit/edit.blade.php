...
@section('content')
<div class="max-w-[95rem] mx-auto" x-data="penerimaanFormEdit()">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-900 flex items-center gap-3">
                <div class="w-1.5 h-6 bg-honda-red rounded-full"></div>
                Edit Dokumen Penerimaan Grup Unit Masuk
            </h2>
        </div>
    ...
    <form @submit.prevent="submitUpdatedAllData">
        @csrf
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="text" x-model="header.no_bukti" readonly class="w-full bg-gray-100 border border-gray-200 text-gray-600 font-bold font-mono rounded-lg p-2.5 text-sm cursor-not-allowed outline-none">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="no_kendaraan" x-model="header.no_kendaraan" required class="w-full border border-gray-300 rounded-lg p-2 text-sm outline-none focus:border-honda-red uppercase">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Ekspedisi Pengirim</label>
                    <input type="text" name="ekspedisi" x-model="header.ekspedisi" required class="w-full border border-gray-300 rounded-lg p-2 text-sm outline-none focus:border-honda-red uppercase">
                </div>
                <div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-6">
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="py-2 px-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a x-show="item.id" :href="`/transaction/motor-unit/${item.id}/print`" target="_blank" class="text-emerald-500 hover:text-emerald-700 font-bold" title="Cetak Unit">Cetak</a>
                                        <button type="button" @click="removeItem(index)" class="text-red-500 hover:text-red-700 font-bold" title="Hapus Unit">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        </template>
                        <tr x-show="details.length === 0">
                            <td colspan="9" class="py-8 text-center text-gray-400 italic">Belum ada unit dalam grup penerimaan ini.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex justify-end gap-3 border-t border-gray-100 pt-6 mt-6">
                ...
                <button type="submit" class="bg-blue-600 text-white font-bold py-2.5 px-8 rounded-lg text-sm hover:bg-blue-700 flex items-center gap-2" :disabled="isSubmitting || details.length === 0">
                    ...
                    <span x-text="isSubmitting ? 'Memproses...' : 'Perbarui Dokumen Grup'"></span>
                </button>
            </div>
        </div>
    </form>
...
<script>
    function penerimaanFormEdit() {
        return {
            ...
            init() {
                this.header.no_bukti = this.penerimaan.no_bukti;
                this.header.tanggal = this.penerimaan.tanggal;
                this.header.no_kendaraan = this.penerimaan.no_kendaraan;
                this.header.ekspedisi = this.penerimaan.ekspedisi;
                this.header.no_sj = this.penerimaan.no_sj;
                this.header.no_nd = this.penerimaan.no_nd || '';
                this.header.no_so = this.penerimaan.no_so || '';

                if (this.penerimaan.motor_units) {
                    this.penerimaan.motor_units.forEach(u => {
                        let display = u.posisi_stok;
                        if (u.posisi_stok === 'POP' && u.lokasi_pop_id) {
                            let matchedPop = this.pops.find(x => x.id == u.lokasi_pop_id);
                            display = 'POP - ' + (matchedPop ? matchedPop.nama_sales : 'Unknown');
                        }
                        this.details.push({
                            id: u.id,
                            posisi_stok: u.posisi_stok,
                            lokasi_pop_id: u.lokasi_pop_id || '',
                            motor_type_id: u.motor_type_id,
                            nama_type: u.type ? u.type.nama_type : '',
                            kode_tipe: u.type ? u.type.kode_tipe : '',
                            motor_color_id: u.motor_color_id,
                            warna: u.color ? u.color.warna : '',
                            kode_warna: u.color ? u.color.kode_warna : '',
                            no_kunci: u.no_kunci,
                            no_seri_kunci: u.no_seri_kunci,
                            no_mesin: u.no_mesin,
                            no_rangka: u.no_rangka,
                            tahun_pembuatan: u.tahun_pembuatan,
                            no_accu: u.no_accu,
                            posisi_display: display
                        });
                    });
                }
            },
            ...
            pushUnitToTable() {
                if (!this.form.motor_type_id || !this.form.motor_color_id || !this.form.no_mesin || !this.form.no_rangka || !this.form.no_kunci || !this.form.no_seri_kunci) {
                    Swal.fire({ icon: 'error', title: 'Data Belum Lengkap', text: 'Semua kolom wajib diisi.' });
                    return;
                }
            ...
            submitUpdatedAllData() {
                ...
                fetch('/transaction/motor-unit/' + this.penerimaan.id, {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({ icon: 'success', title: 'Berhasil Diperbarui!', text: data.message, showConfirmButton: true })
                        .then(() => {
                ...
                .catch(error => {
                    ...
                    Swal.fire({ icon: 'error', title: 'Gagal Memperbarui', text: msg });
                });
            }
        }
    }
