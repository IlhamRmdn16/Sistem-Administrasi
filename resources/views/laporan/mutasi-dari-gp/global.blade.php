...
<div class="max-w-5xl mx-auto">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1 ml-4">Rekapitulasi total mutasi unit keluar dari Showroom GP berdasarkan tipe motor.</p>
        </div>
        <a href="{{ route('laporan.mutasi-dari-gp.print-global', request()->all()) }}" target="_blank" class="bg-gray-800 text-white font-bold py-2.5 px-6 rounded-lg shadow-md hover:bg-gray-900 transition-all flex items-center gap-2">
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6">
        <form action="{{ route('laporan.mutasi-dari-gp.global') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-3">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-8 text-center text-gray-500 italic">Tidak ada data motor keluar dari GP pada periode ini.</td>
                        </tr>
                    @endforelse
                </tbody>
                @if(count($data) > 0)
