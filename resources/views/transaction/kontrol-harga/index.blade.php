...
<div>
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6">
        <form action="{{ route('kontrol-harga.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-4">
    ...
    <form action="{{ route('kontrol-harga.store') }}" method="POST">
        @csrf
                        ...
                        @forelse($spks as $spk)
                            ...
                            <tr class="hover:bg-blue-50/50 group">
                                ...
                                <td class="p-2 border-r border-gray-100 bg-white group-hover:bg-blue-50/50 sticky left-24 z-10">
                                    <a href="{{ route('kontrol-harga.print-options', $spk->id) }}" class="text-xs font-bold text-blue-600 hover:text-blue-800 hover:underline uppercase block truncate w-44" title="Klik untuk Cetak Kwitansi">
                                        {{ $spk->nama_stnk }}
                                    </a>
                                </td>
