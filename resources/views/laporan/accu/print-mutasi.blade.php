...
<head>
    <meta charset="UTF-8">
    <title>Cetak Mutasi Penjualan Accu</title>
    <style>
...
<body onload="window.print()">
    <div class="header">
        <h2>Laporan Kontrol Accu - Mutasi Penjualan</h2>
        <div>Periode Surat Jalan: {{ \Carbon\Carbon::parse($dari_tanggal)->format('d/m/Y') }} s/d {{ \Carbon\Carbon::parse($sampai_tanggal)->format('d/m/Y') }}</div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 20%; text-align: left;">No. Accu</th>
                <th style="width: 15%;">Tanggal SJK</th>
                <th style="width: 12%;">No. Kunci</th>
                <th style="width: 23%; text-align: left;">Tipe Motor</th>
        ...
        <tbody>
            @forelse($reports as $index => $row)
                <tr>
                    ...
                    <td class="text-center">{{ \Carbon\Carbon::parse($row->tanggal_sjk)->format('d/m/Y') }}</td>
                    <td class="text-center" style="font-weight: bold;">{{ $row->no_kunci }}</td>
                    <td>{{ $row->nama_type }}</td>
                    <td style="font-weight: bold; text-transform: uppercase;">{{ $row->nama_pemohon }}</td>
                </tr>
            @empty
                <tr><td colspan="6" class="text-center">Tidak ada data mutasi penjualan.</td></tr>
            @endforelse
        </tbody>
    </table>
</body>
