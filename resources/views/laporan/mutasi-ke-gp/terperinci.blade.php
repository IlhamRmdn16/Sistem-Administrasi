...
<div class="max-w-[95rem] mx-auto">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1 ml-4">Rincian fisik unit motor yang dimutasi ke Showroom GP.</p>
        </div>
        <a href="{{ route('laporan.mutasi-ke-gp.print-terperinci', request()->all()) }}" target="_blank" class="bg-gray-800 text-white font-bold py-2.5 px-6 rounded-lg shadow-md hover:bg-gray-900 transition-all flex items-center gap-2">
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6 flex flex-col lg:flex-row justify-between items-center gap-4">
        <form action="{{ route('laporan.mutasi-ke-gp.terperinci') }}" method="GET" class="w-full flex flex-col sm:flex-row items-center gap-3">
            ...
            <table class="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                    <tr class="bg-slate-50 border-b border-gray-100 text-xs uppercase tracking-wider text-gray-500">
                        ...
                        <th class="py-4 px-4 font-semibold border-b border-gray-200">Lokasi Asal</th>
                ...
                <tbody class="text-sm">
                            ...
                            <tr class="hover:bg-slate-50/30 transition-colors border-b border-gray-100">
                                @if($subIndex === 0)
                                    ...
                                    <td rowspan="{{ $totalRows }}" class="py-3 px-4 font-bold text-gray-800 align-top border-r border-gray-100 bg-slate-50/50">
                                        @if($mutasi->lokasi_asal === 'POP' && $mutasi->asalPop)
                                            POP - {{ $mutasi->asalPop->nama_sales }}
                                        @else
                                            {{ $mutasi->lokasi_asal }}
                                        @endif
                                    </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="py-8 text-center text-gray-500 italic border-b border-gray-100">Tidak ada rincian mutasi ke Showroom GP pada periode ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
