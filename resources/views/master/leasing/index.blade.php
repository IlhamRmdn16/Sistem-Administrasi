...
@section('content')
<div x-data="leasingManager()" @keydown.escape.window="isCreateModalOpen = false; isEditModalOpen = false">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        <div class="p-4 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50">
            <form action="{{ route('leasing.index') }}" method="GET" class="w-full flex flex-col sm:flex-row items-center gap-3">
                <div class="relative w-full sm:w-96">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari kode atau nama leasing..."
                ...
                <a href="{{ route('leasing.index') }}" class="text-center bg-gray-100 text-gray-600 font-semibold px-4 py-2 rounded-lg text-sm hover:bg-gray-200 transition-colors w-full sm:w-auto">Reset</a>
            </form>
            <div class="text-sm text-gray-500 hidden sm:block">
                Menampilkan <span class="font-bold text-gray-800">{{ $leasings->firstItem() ?? 0 }}</span> - <span class="font-bold text-gray-800">{{ $leasings->lastItem() ?? 0 }}</span> dari <span class="font-bold text-gray-800">{{ $leasings->total() }}</span> data
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-gray-100 text-xs uppercase tracking-wider text-gray-500">
                        ...
                        <th class="py-4 px-6 font-semibold">Info Penagihan (Subsidi)</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($leasings as $l)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="py-4 px-6 text-sm font-bold text-gray-800">{{ $l->kode_leasing }}</td>
                            <td class="py-4 px-6 text-sm font-bold text-gray-800">{{ $l->nama_leasing }}</td>
                            <td class="py-4 px-6 text-sm text-gray-600 max-w-xs truncate">{{ $l->alamat ?? '-' }}</td>
                            <td class="py-4 px-6 text-xs text-gray-600">
                                <div class="mb-1"><span class="font-semibold">Sub 1:</span> {{ $l->keterangan_penagihan_1 ?? '-' }}</div>
                                <div><span class="font-semibold">Sub 2:</span> {{ $l->keterangan_penagihan_2 ?? '-' }}</div>
                            </td>
                            <td class="py-4 px-6">
                                <div class="flex items-center justify-center gap-3">
                                    ...
                                    <button type="button" onclick="confirmDeleteAjax({{ $l->id }}, this)" class="text-red-500 hover:text-red-700 transition-colors" title="Hapus">
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="isCreateModalOpen"
                ...
                <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-slate-50">
                    <h3 class="text-lg font-bold text-gray-900">Tambah Data Leasing</h3>
                ...
                <div class="p-6 sm:p-8 max-h-[80vh] overflow-y-auto custom-scrollbar">
                    <form @submit.prevent="submitCreate">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Kode Leasing <span class="text-red-500">*</span></label>
                                <input type="text" name="kode_leasing" placeholder="Contoh: OTO" required class="w-full border border-gray-300 focus:border-honda-red focus:ring-4 focus:ring-red-50 rounded-lg shadow-sm py-2.5 px-4 outline-none transition-all text-gray-800 uppercase">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Leasing <span class="text-red-500">*</span></label>
                                <input type="text" name="nama_leasing" placeholder="Contoh: OTO Finance" required class="w-full border border-gray-300 focus:border-honda-red focus:ring-4 focus:ring-red-50 rounded-lg shadow-sm py-2.5 px-4 outline-none transition-all text-gray-800">
                            </div>

                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Alamat Lengkap (Opsional)</label>
                            ...
                            <div class="md:col-span-2 mt-2">
                                <h3 class="text-sm font-bold text-gray-800 border-b border-gray-100 pb-2 mb-4">Informasi Penagihan (Opsional)</h3>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Keterangan Subsidi 1</label>
                                <input type="text" name="keterangan_penagihan_1" placeholder="Informasi tagihan / PO 1" class="w-full border border-gray-300 focus:border-honda-red focus:ring-4 focus:ring-red-50 rounded-lg shadow-sm py-2.5 px-4 outline-none transition-all text-gray-800">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Keterangan Subsidi 2</label>
                                <input type="text" name="keterangan_penagihan_2" placeholder="Informasi tagihan / PO 2" class="w-full border border-gray-300 focus:border-honda-red focus:ring-4 focus:ring-red-50 rounded-lg shadow-sm py-2.5 px-4 outline-none transition-all text-gray-800">
                            </div>
                        </div>
    ...
    <div x-show="isEditModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="isEditModalOpen"
                ...
                <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-slate-50">
                    <h3 class="text-lg font-bold text-gray-900">Edit Data Leasing</h3>
                ...
                <div class="p-6 sm:p-8 max-h-[80vh] overflow-y-auto custom-scrollbar">
                    <form @submit.prevent="submitEdit">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Kode Leasing <span class="text-red-500">*</span></label>
                                <input type="text" name="kode_leasing" x-model="eKode" required class="w-full border border-gray-300 focus:border-honda-red focus:ring-4 focus:ring-red-50 rounded-lg shadow-sm py-2.5 px-4 outline-none transition-all text-gray-800 uppercase">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Leasing <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Keterangan Subsidi 1</label>
                                <input type="text" name="keterangan_penagihan_1" x-model="eKet1" class="w-full border border-gray-300 focus:border-honda-red focus:ring-4 focus:ring-red-50 rounded-lg shadow-sm py-2.5 px-4 outline-none transition-all text-gray-800">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Keterangan Subsidi 2</label>
                                <input type="text" name="keterangan_penagihan_2" x-model="eKet2" class="w-full border border-gray-300 focus:border-honda-red focus:ring-4 focus:ring-red-50 rounded-lg shadow-sm py-2.5 px-4 outline-none transition-all text-gray-800">
                            </div>
                        </div>
...
<script>
    function leasingManager() {
        return {
            ...
            openEditModal(l) {
                this.eId = l.id;
                this.eKode = l.kode_leasing;
                this.eNama = l.nama_leasing;
                this.eAlamat = l.alamat;
                this.eKet1 = l.keterangan_penagihan_1;
                this.eKet2 = l.keterangan_penagihan_2;
    ...
    function confirmDeleteAjax(id, button) {
        Swal.fire({
            ...
            text: "Data Leasing ini akan dihapus permanen!",
            icon: 'warning',
