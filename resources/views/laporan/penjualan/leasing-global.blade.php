...
<div class="max-w-7xl mx-auto">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-900 flex items-center gap-3">
                ...
                Laporan Penjualan Leasing - Global
            </h2>
            <p class="text-sm text-gray-500 mt-1 ml-4">Analisis performa penjualan unit motor berdasarkan kemitraan perusahaan finansial (Leasing).</p>
        </div>
        <a href="{{ route('laporan.penjualan.leasing-global.print', request()->all()) }}" target="_blank" class="bg-gray-800 text-white font-bold py-2.5 px-6 rounded-lg shadow-md hover:bg-gray-900 transition-all flex items-center gap-2">
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6">
        <form action="{{ route('laporan.penjualan.leasing-global') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            ...
            <div>
                ...
                <select name="leasing_id" class="w-full border border-gray-300 rounded-lg p-2 text-sm outline-none bg-white focus:border-honda-red">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse whitespace-nowrap">
                <thead class="bg-honda-red text-white text-[11px] uppercase tracking-wider">
                    <tr>
                        <th class="py-4 px-4 border-r border-red-500">Nama Leasing</th>
                    ...
                    @forelse($reports as $leasingName => $items)
                        ...
                        @foreach($items as $row)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="py-3 px-4 font-bold text-gray-900 border-r border-gray-100 uppercase">{{ $leasingName }}</td>
                        ...
                        <tr class="bg-gray-50 font-bold text-gray-900 border-y border-gray-200">
                            <td colspan="3" class="py-3 px-4 text-right border-r border-gray-200 uppercase text-xs">Sub Total {{ $leasingName }} :</td>
                        ...
                        @endphp
                    @empty
                        <tr><td colspan="7" class="py-12 text-center text-gray-400 italic">Tidak ada transaksi penjualan pada rentang tanggal ini.</td></tr>
                    @endforelse
                </tbody>
                @if($reports->count() > 0)
                <tfoot class="bg-gray-800 text-white font-black text-sm border-t-2 border-gray-900">
                    <tr>
                        <td colspan="3" class="py-4 px-4 text-right uppercase bg-gray-900 border-r border-gray-700">Grand Total Keseluruhan :</td>
