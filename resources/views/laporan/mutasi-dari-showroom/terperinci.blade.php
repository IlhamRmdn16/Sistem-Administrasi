...
<div class="max-w-[95rem] mx-auto">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-900 flex items-center gap-3">
                ...
                Laporan Terperinci Mutasi Dari Showroom
            </h2>
            <p class="text-sm text-gray-500 mt-1 ml-4">Rincian fisik unit motor yang dimutasi keluar dari Showroom Pusat.</p>
        </div>
        <a href="{{ route('laporan.mutasi-dari-showroom.print-terperinci', ['dari_tanggal' => $dari_tanggal, 'sampai_tanggal' => $sampai_tanggal]) }}" target="_blank" class="bg-gray-800 text-white font-bold py-2.5 px-6 rounded-lg shadow-md hover:bg-gray-900 transition-all flex items-center gap-2">
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6 flex flex-col lg:flex-row justify-between items-center gap-4">
        <form action="{{ route('laporan.mutasi-dari-showroom.terperinci') }}" method="GET" class="w-full flex flex-col sm:flex-row items-center gap-3">
                        ...
                        @foreach($mutasi->details as $subIndex => $detail)
                            ...
                            <tr class="hover:bg-slate-50/30 transition-colors border-b border-gray-100">
                                @if($subIndex === 0)
                                    ...
                                    <td rowspan="{{ $totalRows }}" class="py-3 px-4 text-center text-gray-800 font-semibold align-top border-r border-gray-100 bg-slate-50/50">
                                        {{ $mutasi->lokasi_tujuan === 'POP' ? 'POP - ' . ($mutasi->tujuanPop->nama_sales ?? '') : $mutasi->lokasi_tujuan }}
                                    </td>
                                @endif
                                ...
                                <td class="py-3 px-4 text-center text-gray-600">{{ $unit->tahun_pembuatan ?? '-' }}</td>
                            </tr>
                        @endforeach
                    @empty
                        <tr>
                            <td colspan="10" class="py-8 text-center text-gray-500 italic border-b border-gray-100">Tidak ada rincian mutasi unit keluar dari Showroom pada periode ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
