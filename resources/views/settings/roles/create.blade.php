...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1 ml-9">Definisikan jabatan dan atur hak aksesnya.</p>
        </div>
    </div>
    ...
    <form action="{{ route('roles.store') }}" method="POST">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-6">
            ...
            <input type="text" name="name" required placeholder="Contoh: Admin GP, Finance, Sales Counter..." class="w-full md:w-1/2 border border-gray-300 rounded-lg p-3 outline-none focus:border-honda-red font-bold uppercase text-gray-800">
        </div>
                        ...
                        @foreach($permissions as $key => $label)
                            <label class="flex items-start gap-3 cursor-pointer group">
                                <div class="relative flex items-center mt-0.5">
                                    <input type="checkbox" name="permissions[]" value="{{ $key }}" class="peer w-5 h-5 cursor-pointer appearance-none rounded border-2 border-gray-300 checked:bg-honda-red checked:border-honda-red transition-all">
