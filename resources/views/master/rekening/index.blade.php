...
<div x-data="rekeningManager('{{ $autoKodeRekening }}')" @keydown.escape.window="isCreateModalOpen = false; isEditModalOpen = false">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="p-4 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50">
            <form action="{{ route('rekening.index') }}" method="GET" class="w-full sm:w-96 relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau nomor rekening..."
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="py-4 px-6">
                                <div class="flex items-center justify-center gap-3">
                                    ...
                                    <form action="{{ route('rekening.destroy', $r->id) }}" method="POST" onsubmit="confirmDelete(event, this)">
                                        @csrf @method('DELETE')
    ...
    <div x-show="isEditModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="isEditModalOpen"
                ...
                <div class="p-6">
                    <form :action="'/master/rekening/' + eId" method="POST" class="space-y-5">
                        @csrf @method('PUT')
                        <div>
