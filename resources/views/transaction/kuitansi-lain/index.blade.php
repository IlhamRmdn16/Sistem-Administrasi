...
<div x-data="kllManager()">
    ...
    <!-- TOMBOL NAVIGASI KHUSUS MODE HP (Hanya Muncul di Layar Kecil) -->
    <div class="flex lg:hidden gap-2 mb-6 bg-gray-100 p-1 rounded-xl">
        <button @click="mobileView = 'form'" :class="mobileView === 'form' ? 'bg-white text-honda-red shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="flex-1 py-2.5 rounded-lg font-bold text-sm transition-all duration-200">
            📝 Isi Formulir
        </button>
        <button @click="mobileView = 'riwayat'" :class="mobileView === 'riwayat' ? 'bg-white text-honda-red shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="flex-1 py-2.5 rounded-lg font-bold text-sm transition-all duration-200">
            🗂️ Lihat Riwayat
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <!-- KOLOM KIRI: FORMULIR -->
        <div :class="mobileView === 'form' ? 'block' : 'hidden lg:block'" class="lg:col-span-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm sticky top-6">
            ...
            <form :action="isEditMode ? '/transaction/kuitansi-lain/' + eId : '{{ route('kuitansi-lain.store') }}'" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="alamat" x-model="eAlamat" placeholder="Nama Jalan / Kampung" class="w-full border border-gray-300 rounded p-2 text-xs outline-none focus:border-honda-red mb-2 uppercase">
        ...
        <div :class="mobileView === 'riwayat' ? 'block' : 'hidden lg:block'" class="lg:col-span-8 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-4 bg-slate-50 border-b border-gray-100">
                <form action="{{ route('kuitansi-lain.index') }}" method="GET" class="flex flex-wrap items-end gap-3">
                    <div class="flex-1 min-w-[180px]">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Bukti / Nama..." class="w-full border border-gray-300 rounded py-2 px-3 text-xs outline-none focus:border-honda-red">
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Periode</label>
                        <div class="flex items-center gap-2">
                            <input type="date" name="start_date" value="{{ request('start_date') }}" class="border border-gray-300 rounded py-2 px-2 text-xs outline-none focus:border-honda-red">
                            <span class="text-xs text-gray-400">s/d</span>
                            <input type="date" name="end_date" value="{{ request('end_date') }}" class="border border-gray-300 rounded py-2 px-2 text-xs outline-none focus:border-honda-red">
                        </div>
                    </div>
                    <div>
                    ...
                    <div class="flex gap-2 w-full sm:w-auto">
                        <button type="submit" class="flex-1 sm:flex-none bg-gray-800 text-white font-bold px-4 py-2 rounded text-xs hover:bg-gray-900">Filter</button>
                        <a href="{{ route('kuitansi-lain.index') }}" class="flex-1 sm:flex-none text-center bg-gray-200 text-gray-700 font-bold px-3 py-2 rounded text-xs hover:bg-gray-300">Reset</a>
                    </div>
                </form>
            </div>
                            ...
                            <tr :class="eId == {{ $item->id }} ? 'bg-blue-50/70' : 'hover:bg-slate-50'">
                                ...
                                <td class="py-3 px-4">
                                    <div class="flex items-center justify-center gap-2.5 relative z-10">
                                        ...
                                        <!-- Tombol Edit -->
                                        <button type="button" @click="loadToForm({{ $item }})" class="text-blue-500 hover:text-blue-700" title="Edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>

                                        <!-- Tombol Hapus (Dipisah dari Form untuk menghindari konflik klik) -->
                                        <button type="button" @click="deleteData({{ $item->id }})" class="text-red-500 hover:text-red-700" title="Hapus">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>

                                        <!-- Hidden Form Hapus -->
                                        <form id="delete-form-{{ $item->id }}" action="{{ route('kuitansi-lain.destroy', $item->id) }}" method="POST" class="hidden">
                                            @csrf 
                                            @method('DELETE')
                                        </form>
                                    </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('kllManager', () => {
            const urlParams = new URLSearchParams(window.location.search);
            const hasFilter = urlParams.has('search') || urlParams.has('page') || urlParams.has('start_date');

            return {
                mobileView: hasFilter ? 'riwayat' : 'form', 
                isEditMode: false,
                eId: '',
                ...
                // Logika Hapus dipindah ke dalam Alpine agar terhindar dari konflik eksekusi
                deleteData(id) {
                    Swal.fire({
                        title: 'Hapus Kuitansi?',
                        text: "Nomor kuitansi ini akan dikembalikan dan dapat digunakan ulang.",
                        icon: 'warning',
